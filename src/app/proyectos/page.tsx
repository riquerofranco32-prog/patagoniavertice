...
export const metadata = {
  ...
  description: "Conocé todos los proyectos inmobiliarios de Patagonia Vértice en Neuquén y la Patagonia Argentina.",
};
...
const demos = [
  {
    id: "demo-1", titulo: "Altos del Limay", barrio: "Confluencia", ciudad: "Neuquén", tipo: "desarrollo", estado: "en_construccion",
    imagenes: ["https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=800&q=80"],
    descripcion: "Complejo residencial en altura con vistas panorámicas al río Limay. Unidades de 2 y 3 ambientes con amenities.",
    precio: null, moneda: "USD",
  },
  {
    id: "demo-2", titulo: "Parque Patagónico", barrio: "Centenario", ciudad: "Neuquén", tipo: "lote", estado: "disponible",
    imagenes: ["https://images.unsplash.com/photo-1501854140801-50d01698950b?w=800&q=80"],
    descripcion: "Loteo premium en entorno natural. 80 lotes de entre 500 y 1.200 m² con acceso a áreas verdes.",
    precio: null, moneda: "USD",
  },
  {
    id: "demo-3", titulo: "Vértice Confluencia", barrio: "Villa María", ciudad: "Neuquén", tipo: "desarrollo", estado: "vendido",
    imagenes: ["https://images.unsplash.com/photo-1470770903676-69b98201ea1c?w=800&q=80"],
    descripcion: "Desarrollo de viviendas unifamiliares en el corazón residencial de Neuquén. Diseño contemporáneo.",
    precio: null, moneda: "USD",
  },
];
...
export default async function ProyectosPage() {
  ...
  const items = propiedades.length > 0 ? propiedades : (demos as unknown as Propiedad[]);
      ...
      <div className="bg-tierra pt-40 pb-24 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="font-body text-dorado text-[11px] tracking-[0.35em] uppercase">Portafolio</span>
          </div>
          <h1
          ...
          <p className="font-body text-crema/35 text-[15px] leading-relaxed max-w-md">
            Desarrollos que transforman regiones y generan valor real en la Patagonia.
          </p>
        </div>
      ...
      <section className="py-24 bg-crema">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
            {items.map((p, i) => {
              const imagen = p.imagenes?.[0] ?? `https://placehold.co/600x400/1C1A17/B8965A?text=${encodeURIComponent(p.titulo)}`;
              ...
              return (
                <Link key={p.id} href={`/proyectos/${p.id}`} className="group block">
                  ...
                  <div className="relative overflow-hidden aspect-[4/3] mb-5 bg-arena">
                    ...
                    />
                    {/* Overlay */}
                    <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(28,25,22,0.5) 0%, transparent 50%)" }} />
                    {/* Number */}
                    <div
                    ...
                    {badge && (
                      <div className="absolute bottom-4 left-4">
                        <span className={`font-body text-[9px] tracking-[0.25em] uppercase ${badge.color} bg-tierra/80 backdrop-blur-sm px-3 py-1.5`}>
                          {badge.label}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="font-body text-dorado text-[10px] tracking-[0.25em] uppercase mb-2">
                    {p.tipo ? tipoLabel[p.tipo] : "Proyecto"} · {p.ciudad ?? "Neuquén"}
                  </div>
                  <h3
                  ...
                  {p.barrio && (
                    <p className="font-body text-tierra/40 text-sm">{p.barrio}</p>
                  )}
                  {p.precio && (
                  ...
                  <div className="mt-4 flex items-center gap-2 text-tierra/30 group-hover:text-dorado transition-colors duration-300">
                    <span className="font-body text-[10px] tracking-[0.2em] uppercase">Ver proyecto</span>
          ...
          <div className="mt-20 pt-16 border-t border-tierra/8 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="font-body text-tierra/40 text-sm">Contanos qué necesitás y encontramos la opción ideal.</p>
            </div>
            <Link
              href="/contacto"
