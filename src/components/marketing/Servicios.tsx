      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-tierra/10">
          {servicios.map((s) => (
            <div
              key={s.titulo}
              className="bg-[#FAFAF8] p-10 group hover:bg-tierra transition-colors duration-500"
            >
              <div className="text-dorado mb-6 group-hover:text-dorado transition-colors">
                {s.icon}
              </div>
              <h3 className="font-display text-tierra text-xl font-light mb-4 group-hover:text-crema transition-colors">
                {s.titulo}
              </h3>
              <p className="font-body text-tierra/50 text-sm leading-relaxed group-hover:text-crema/50 transition-colors">
                {s.descripcion}
              </p>
              <div className="mt-8 h-px w-0 bg-dorado group-hover:w-10 transition-all duration-500" />
            </div>
          ))}
        </div>
      </div>
